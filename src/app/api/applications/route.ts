...
export async function POST(req: NextRequest) {
  try {
    ...
    if (tooManyApplicationSubmissions(ip, body.deviceHash)) return NextResponse.json({ ok: false, message: 'تم إرسال عدد كبير من الطلبات. حاول لاحقًا.' }, { status: 429 });
    const result = await createApplication(body, { ip, userAgent: req.headers.get('user-agent') });
    return NextResponse.json({ ok: true, applicationNo: result.app.application_no, status: result.app.status, flags: result.flags.length });
  } catch (error: any) { return NextResponse.json({ ok: false, message: error?.message || 'حدث خطأ أثناء إرسال الطلب' }, { status: 400 }); }
}
